...
class DCGAN(object):
  ...
  def build_model(self):
    ...
    #这里使用的是原始gan的loss函数，改成improve wgan的loss函数
    ...
    x_hat = epsilon * inputs + (1 - epsilon) * self.G  # 在Pdata 和pg中间sample penalty
    d_1, d_hat = self.discriminator(x_hat, self.y, reuse=True)
      ...
      for idx in range(0, batch_idxs):
        ...
        if np.mod(counter, 100) == 1:
          ...
          if config.dataset == 'mnist':
            ...
            try:
              # sample_z1 = np.random.uniform(-1, 1, size=(64, self.z_dim))
              samples, d_loss, g_loss = self.sess.run(
                [self.sampler, self.d_loss, self.g_loss],
                feed_dict={
                    self.z: sample_z,
